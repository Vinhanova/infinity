...
const InvestmentsPage: FC = () => {
  ...
  const assetsArray = _.chain(purchasedAssetsList)
    .map(asset => {
      const quantity = userTickersData[asset.id]?.quantity || 0
      if (quantity === 0) return null // Filter out assets with quantity 0
      return { ...asset, quantity } // Return asset with quantity included
    })
    .compact() // Remove any null values from the array
    .value()

  // Step 2: Sort by the computed value (price * quantity) in descending order
  const sortedAssetsArray = assetsArray.sort((a, b) => {
